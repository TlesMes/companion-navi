...
1. **D4 결정 완료: 실시간 음성 API(Gemini Live 등) 채택하지 않음.** 직렬 스트리밍 파이프라인으로 간다.
   - 근거: ① 목소리는 데몬의 자산이어야 한다(벤더 내장 음성에 묶이면 "그 애"의 연속성이 벤더에 종속됨) ② 즉답(0.3초급)은 불필요, 첫 오디오 ~1초면 충분 ③ 15분 세션 제한·세션 매니저 재설계 회피.
...
2. **목소리도 데몬의 자산이다.** (v2 신설) 음색(voice_profile)은 "그 애"의 정체성이므로 단일 고정이며, TTS 벤더는 어댑터 뒤에 숨긴다. 두뇌를 갈아껴도, TTS 벤더를 갈아껴도 같은 목소리로 말해야 한다.
...
pick_topic(memory_snapshot, weather, time_of_day) → topic_hint
```
...
# voice_profile은 전 모드 공통 단일값 — "그 애"의 목소리 (설계 원칙 2)
```
...
| name | 예: 말동무 / 반려자 |
| min_intimacy | 이 페르소나가 적용되는 친밀도 하한 |
| system_prompt | 해당 페르소나의 시스템 프롬프트 본문 |
...
voice_profile: 단일 고정 — "그 애"의 목소리. 모드 무관, 벤더 교체 후에도 유지 노력
```
...
| D3 | TTS 벤더 | **격상 — 가장 중요한 결정.** 음색=제품 정체성. 후보: Cartesia Sonic(TTFA 최속) vs ElevenLabs Flash(품질, 고가). 스펙이 아니라 직접 듣고 결정할 것 |
...
| D9 | 친밀도 산식 | 보류. 단조증가 방지(감쇠)·페르소나 전환 히스테리시스 포함해 설계할 것 |
