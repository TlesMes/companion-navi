...
from navi.ear.wakeword import FakeWakeWord, PorcupineWakeWord, WakeWord
...
def create_wakeword(kind: str = "porcupine", **kwargs) -> WakeWord:
    """웨이크워드 어댑터 팩토리 — 벤더 종속 금지. fake는 무의존(테스트·키 없는 환경)."""
    if kind == "porcupine":
        return PorcupineWakeWord(**kwargs)
    if kind == "fake":
        return FakeWakeWord(**kwargs)
    raise ValueError(f"알 수 없는 wakeword kind: {kind!r} (porcupine | fake)")
