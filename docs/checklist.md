...
- [ ] **A3. 음성으로 먼저 말 걸기 E2E** — **보류 해제(2026.08.28). 청취만 남았다.**
      (보류 사유였던 "3층 `topic_feed`가 빈 더미"는 D13 PR3(#47)로 해소.)
      - **마이크 불필요** — 선제 발화엔 STT가 안 낀다(마이크는 *반응* 측정 = B2 영역).
        스피커도 청취용일 뿐: `_synth`/`_play`가 분리돼 **합성만 헤드리스로** 돌아간다.
      - **이미 확인된 것:** 전 구간 통과(실 RSS → gemini 요약 → 두뇌 → 발화, **사람이 손댄 텍스트
        0**) · 한국어 합성 4건(supertonic RTF 0.33~0.41, 숫자·외래어가 순한글 대조군과 같은 길이
        대역) · 일본어 합성 2건(gptsovits RTF 1.78~2.01, 언어 판정 `ja` 일관).
      - **준비물** (`config.local.yaml`, 끝나면 지울 것):
        `feed.summarizer.vendor: anthropic`(**기본 `echo`면 요약이 안 된다**) ·
        `feed.interests`에 관심사 RSS(살아있는 목록은 progress.md 운영 메모) ·
        `proactive.min_gap_s: 0` + `base_interval_s: 30`(기본값이면 첫 발화까지 **48분**,
        이 값이면 20~40초 — 시뮬레이션 확인). 실행은 `run_navi.ps1 -Db logs\a3.db`(본 기억 격리),
        `--wakeword` 불요. **취침창 23:00~07:00엔 선제 발화가 아예 안 나간다.**
      - **볼 것:** ①되묻는가(텍스트 실측 3/4이 되물었다 — turn_assembly §5 손잡이 ②, **청취 후
        튜닝하기로**) ②숫자·외래어 발음(합성 길이는 정상, 발음은 미확인) ③아나운서인가 식구인가
        (재료가 뉴스체인데 두뇌가 충분히 입말로 바꾸는지) ④첫 오디오까지 체감 시간.
