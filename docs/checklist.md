...
- [x] **A3. 음성으로 먼저 말 걸기 E2E — 완료(2026.08.30).**
      **데몬이 실제로 스스로 말했다.** fake mouth(소리 없음)로 진짜 `_run` + tick 루프를 돌려
      ...
      - **★ 이 실행이 PR3의 부팅 즉사 버그를 잡았다** — `_build_feed`가 `user_id` 정의보다
        ...
        부팅 경로(`_run`)를 건너뛴다. 파이썬 컴파일러도 못 잡고 리포에 린터도 없다.
        → **`_run` 스모크가 없다는 구조적 공백**(백로그 등록).
...
- **`_run` 스모크 테스트 부재** — 신설 2026.08.30(A3에서 드러남). 부팅 경로를 타는 테스트가
  하나도 없다: 통합 테스트는 `DaemonCore`에 협력자를 **직접 주입**해 `_run`을 건너뛴다.
  그래서 **411 tests green인데 데몬이 부팅에서 즉사**하는 상태가 머지까지 갔다
  (`_build_feed`를 `user_id` 정의 앞에서 호출 → `UnboundLocalError`).
  파이썬 컴파일러는 못 잡고(지역변수라 문법상 유효) **리포에 린터도 없다**.
  막을 방법 셋: ①`_run`을 fake mouth·fake 두뇌로 도는 스모크 ②`ruff`/`pyflakes`(F823) 도입
  ③실행해 보기(이번엔 이것으로 잡았다). ②가 가장 싸다 — 이 종류를 통째로 막는다.
