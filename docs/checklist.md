...
- [x] **A5. 페르소나 게이팅 실기 확인(E3)** — **통과(2026.07.19).** 확인 9항목 전부 기대대로:
      ...
      **게이팅 로직 검증 완료 — 남은 건 표시 결함뿐(→ E8).**
      ...
      E8 재확인에 쓰고 그 뒤 `rm personas/zz_*.yaml`.

## E. 데몬 기동 정상화 — 진행 중 (2026.07.19 시작, A·B보다 우선)

`--voice` 부팅이 어떤 카드로도 죽던 버그에서 출발한 묶음. **E1·E2·E4·E3 머지 완료 + A5 실기 통과.**
남은 순서: **E8 → E6 → E7 → E5**(E8은 A5 임시 카드가 남아 있는 동안이 재확인이 싸다,
E6는 스크립트 기본값이 실사용으로 자리잡은 뒤, E5는 그 뒤 검토).
...
- [ ] **E8. GUI 표시 결함 2건 (360px 창을 못 견딤)** — A5 실기에서 관측(2026.07.19). 게이팅 로직은
      정상이고 **보여주는 쪽만** 문제다.
