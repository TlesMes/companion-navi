...
- [ ] **A5. 페르소나 게이팅 실기 확인(E3)** — 로직·API는 테스트로 고정됐지만 **회색 처리·사유 툴팁의
      실제 모양**은 GUI 창을 띄워야 보인다. 확인할 것: ① 기본 카드(navi=supertonic)로 부팅 시
      아리스 카드가 회색인가 ② 툴팁에 `-Persona` 재기동 해법이 읽히는가 ③ 회색 항목 클릭 시
      토스트로 사유가 뜨는가(`disabled` 속성을 안 쓴 이유) ④ 톤 칩 단위 비활성(④-b) 렌더링.
      데몬은 [scripts/run_navi.ps1](../scripts/run_navi.ps1), GUI는 `python -m navi.gui`.

## E. 데몬 기동 정상화 — 진행 중 (2026.07.19 시작, A·B보다 우선)

`--voice` 부팅이 어떤 카드로도 죽던 버그에서 출발한 묶음. **E1·E2·E4·E3 머지 완료.**
남은 순서: **E6 → E7 → E5**(E6는 스크립트 기본값이 실사용으로 자리잡은 뒤, E5는 그 뒤 검토).
