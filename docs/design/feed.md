...
- **나비는 바깥소식을 어떻게 아는가? → ⓒ 둘 다로 확정 (2026.08.25).**
  ...
  **근본 원인은 프레이밍이 아니라 세계관 모순이었다** — 카드가 *"집 안이 세상의 전부"* 라고
  ...
  → **두 층에 나눠 넣었다.** ⓐ카드 `background`가 **능력**을 세운다(*"집 안 기기들 사이로 바깥
  이야기가 흘러들어오되, 어디서 온 이야기인지까지는 모른다"* — 마지막 절이 작화를 세계관
  수준에서 막는다). ⓑ`_PROACTIVE_FRAME`이 **이번 건**을 통제한다(출처를 주고 지어내기 금지).
  층이 달라 서로를 대체하지 못한다.
  **실측 확인(2026.08.25, haiku 3건):** 작화 신호 0건. 뉴스 소재에서 나비가 *"스피커에서 뭔가
  새로운 게임기 나온대"* 라고 **카드의 새 경로를 실제로 사용**했고 전언 어법(`~대`)도 유지됐다.
  표본 3건이라 "반례를 못 봤다"이지 증명은 아니다 — A3 실기에서 재확인.
