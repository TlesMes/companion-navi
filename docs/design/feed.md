...
- **나비는 바깥소식을 어떻게 아는가? → ⓒ 둘 다로 확정 (2026.08.25).**
  ...
  **근본 원인은 프레이밍이 아니라 세계관 모순이었다** — 카드가 *"집 안이 세상의 전부"* 라고
  ...
  ⓑ**`_PROACTIVE_FRAME` = 금지(보편).** *"출처를 지어내지 마"* 만 말한다.
  ...
  **실측 확인(2026.08.25, haiku, 카드 2종):**
  ...
  - 부수: 프레임 힌트를 뺀 뒤 나비가 경로를 **자발적으로 언급하진 않는다**(묻기 전엔 사실만).
    자발 언급까지 원하면 카드에 선택 필드(`source_hint`)를 두고 프레임이 보간하는 방법이 있다 —
    그건 카드 스키마 확장이라 별도 결정.
