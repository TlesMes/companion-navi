...
- ② 추출을 저가 LLM 대신 규칙(형태소 빈도)으로? → 한국어 형태소 부담 커서 **LLM 1회**가
  MVP엔 단순. 정확도 튜닝 시 재론.
- **나비는 바깥소식을 어떻게 아는가? → ⓒ 둘 다로 확정 (2026.08.25).**
  (문제: 2026.07.26 실측 — [turn_assembly.md](./turn_assembly.md) §4.1 ①) 선제 발화 시 두뇌가
  출처를 **지어냈다**: "미리 알아봤어"(gemini) · "어제 뉴스에서 봤는데"(haiku). 나비는 카드상
  몸이 없어 집 밖에 못 나가는 정령이라 둘 다 페르소나 위반이고, 카드 `traits`의 *"기억에 없는
  일을 지어내지 않는다"* 를 정면으로 어긴다.
  **근본 원인은 프레이밍이 아니라 세계관 모순이었다** — 카드가 *"집 안이 세상의 전부"* 라고
  바깥소식을 알 **경로 자체를 부정**하는데 피드는 바깥 뉴스를 먹인다. 프레이밍만 고치면 매번
  덧대는 반창고이고, 반응 턴에서 "그거 어떻게 알았어?"를 물으면 카드가 답을 못 한다.
  → **두 층에 나눠 넣되 역할이 겹치지 않게 한다.**
  ⓐ**카드 `background` = 경로(세계관).** 나비는 *"집 안 기기들 사이로 바깥 이야기가
  흘러들어오되, 어디서 온 이야기인지까지는 모른다"* — 마지막 절이 작화를 세계관 수준에서 막는다.
  ⓑ**`_PROACTIVE_FRAME` = 금지(보편).** 두 가지를 막는다 — *"어떻게 알게 됐는지는 굳이 말하지
  말고, 출처를 지어내지도 마."* **자발 언급까지 막는 건 사용자 판단**(2026.08.25): 먼저 출처를
  밝히면 어색하고 발화만 길어진다. 카드 `traits`의 *"짧고 가볍게, 한 번에 1~3문장"* 과도 맞다.
  힌트를 뺀 직후 실측에선 이미 자발 언급이 없었지만 그건 규칙이 아니라 그때 모델의 성향이라
  명시해 뒀다(재료 언어에서 같은 교훈을 얻었다 — §3.3). **물어보면 답하는 건 카드 몫**이다.
  > ⚠ **프레임에 세계관을 넣으면 안 된다.** 한때 프레임에 *"집 안에 흘러든 이야기로 알게 된
  > 거야"* 라고 나비 설정을 박았다가 되돌렸다 — `conductor.py`는 **모든 카드가 공유하는 코드**라
  > 집과 무관한 카드(`example_jp` 등)가 남의 세계관을 뒤집어쓴다. 경로는 카드 소유다.
  > **경로를 안 주는 카드는 출처 없이 사실만 말한다** — 결함이 아니라 정상 폴백이다. 다만
  > *"몸이 없어 밖에 못 나간다"* 처럼 **스스로를 제약하는 카드는 경로도 스스로 줘야 한다.**

  **실측 확인(2026.08.25, haiku, 카드 2종):**
  - 선제 발화 4건(navi 2 + example_kr 2) 전부 **작화 신호 0건**. navi *"닌텐도가 새 기계
    나온다고 하네. 내년 3월이래."* — 전언 어법(`~이래`) 유지.
  - **반응 턴에서 "그거 어떻게 알았어?"로 캐물었을 때 카드 층이 실제로 답했다.** navi:
    *"집 기기들 사이로 흘러든 이야기야. … **정확한 게 뭔지까진 모르지만**"* — background의
    두 절이 그대로 나왔다. 프레임은 억제만 하지 이 답을 못 만든다. **ⓐ가 필요한 이유의 실증.**
  - **"굳이 말하지 마" 추가 후 재확인(카드 2종 × 소재 2종 = 4건): 출처 언급 0건**, 발화도 짧다.
    navi *"닌텐도 새 기계가 내년 3월에 나온대."* / example_kr *"서울이 34도까지 올라갔네."*
  - 기각한 대안: 카드 선택 필드(`source_hint`)를 프레임이 보간해 **자발 언급**을 살리는 안.
    사용자 판단으로 자발 언급 자체를 원하지 않아 카드 스키마를 안 넓혔다.
