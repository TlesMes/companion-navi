...
| **2** | **A3 실기** — **청취만 남았다.** 헤드리스로 전 구간(실 RSS → gemini 요약 → 두뇌 → TTS 합성)이 흘렀고, 한국어(supertonic RTF 0.33)·일본어(gptsovits RTF 1.8~2.0) 합성까지 확인됨. **마이크 불필요**(선제 발화엔 STT가 안 낀다) | — | 소(사용자) |
| 3 | **B5 / D11** — 스케줄 동기화(캘린더 vs 컴패니언 앱) | — | 중 |
| 4 | **B2** — `interaction_log` 축적 → hazard 튜닝 | **시간**(실사용) | 소, 상시 |
| — | *여기서 Phase 3 코드 완료. 기준 충족은 4가 쌓인 뒤* | | |
| 5 | **Phase 4** — 장기기억(D6 sqlite-vec) · 사실 충돌·망각 · D9 친밀도 산식 · 페르소나 전환 히스테리시스 | — | **대** |
| 6 | **D8 확보** — GPU·상시기기 | 돈·하드웨어 | 외부 |
| 7 | **동결 해제** — C1 속도 · C2(D2 STT 선정) · C3 AEC | 6 | 중 |
| 8 | **Phase 5** — barge-in · D12 턴테이킹 · D10 정서 안전 · 고장 모드 · 상시기기 이전 | 6·7 | 대 |

**5(Phase 4)는 6(D8)을 안 기다린다** — 기억·인격은 전부 텍스트 계층이라 GPU와 무관하다.
