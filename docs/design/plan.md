...
> 상세 설계는 [architecture.md](./architecture.md), 벤더 조사·원가는 [vendor_cost.md](./vendor_cost.md) 참조.
...
- **해자:** 데몬 + 누적된 기억 DB. 두뇌를 갈아껴도 나비는 유지된다.
...
> **범위 동결 (2026.06.11):** v1 범위는 아래 로드맵에 적힌 것까지. 이후 새 아이디어는 구현하지 않고 백로그로만 기록한다 — 동결의 핵심은 적게 적는 게 아니라, 적힌 것에 구현 순서가 박혀 있어 새치기가 안 되는 것.

### Phase 0 — 기획·설계 ✅ 완료 (2026.06)
콘셉트 확정, 설계 문서 v2(직렬 스트리밍 + 자체 턴테이킹), 기술조사·원가 시뮬레이션(월 ~$2~5 예상), git repo 구축.

### Phase 1 — 텍스트 뼈대: 나비가 존재하기 시작 ✅ 완료 (2026.06)
음성 없이 CLI 텍스트 대화로 핵심 루프 구축: Conductor(프롬프트 조립) + Brain 어댑터 + 단기기억 + SQLite. 페르소나는 **캐릭터 카드**(배경 서사·성격/말투 규칙·few-shot 대화예시)로 작성해 말투 확립.
**완료 기준:** 껐다 켜도 어제 대화를 기억한다. Brain 어댑터에서 LLM 벤더를 바꿔도 같은 애처럼 말한다.

### Phase 2 — 음성화: 듣고 말하는 존재 🟡 배선 완결, 속도만 동결 (D8 대기)
...
### Phase 3 — 능동성: 먼저 말 거는 존재 🔵 진행 중 (2026.08 현재)
Heartbeat 3층(모드게이트→타이밍→주제) + 스케줄 동기화(D11) + 선제 발화 + **관심사 피드(D13)** — 뉴스·커뮤니티를 주기 수집해 선제 발화 재료로(명시 등록 + 대화 빈도 기반 자동 추출). interaction_log로 응답률/무시율 수집 시작.
**완료 기준:** 자는 시간에 절대 말 안 건다. "더 잘래"가 즉시 먹힌다. 선제 발화 응답률이 무시율보다 높다.

### Phase 4 — 기억·인격 심화: 같이 산 시간이 쌓이는 존재 ⬜ 미착수
장기기억(사실 추출→벡터 검색), 사실 충돌·망각 규칙.
**완료 기준:** 한 달 전 이야기를 자연스럽게 다시 꺼낸다.

> **범위 축소 (2026.08.31): 친밀도 산식(D9)·페르소나 전환을 폐기했다.** 단계별 프로필은 카드
> 작성자에게 "어느 단계의 대화문인지"를 상상하게 만들어 말투 일관성을 해치고, 무엇이 점수를
> 올리고 내리는지는 종이로도 로그로도 정할 수 없었다(측정과 개입이 얽힌다). "톤이 변한다"는
> 체감은 **무드(D14 빠른 경로)가 이미 제공**하고 실기까지 끝났다. 상세 →
> [aliveness.md](./aliveness.md) §3.3. **Phase 4는 이제 온전히 기억 하나의 축이다.**

### Phase 5 — 완성: 같이 살 만한 존재 ⬜ 미착수
barge-in 완성, 턴테이킹 튜닝(D12), 정서 안전장치(D10), 고장 모드(API 장애 시 행동), 상시기기 이전(D8).

---

## 2.1 완성까지 남은 길 (2026.08.25 재정리)

> 실행 뷰는 [checklist.md](../checklist.md), 진행 근거는 [progress.md](../progress.md).
> 이 절은 **무엇이 무엇을 막고 있는지**만 본다.

### 지금 위치

Phase 0·1 완료. Phase 2는 **배선이 다 끝났고 속도만 동결**(마이크→나비 음성 답변이 실동하되
E2E 9.8초, 그중 TTS가 63%). Phase 3은 **3층 중 3층까지 코드가 섰고** 재료 공급(D13)이
PR3까지 끝났고 **A3 실기도 통과했다** — 나비가 실제로 스스로 말한다.

### 남은 작업을 막고 있는 것 셋

**① D8(GPU·상시기기)이 단일 병목이다.** 속도 관련 잔여가 전부 여기로 수렴한다 —
C1(E2E 속도)·C2(D2 스트리밍 STT 선정)·C3(AEC·barge-in), 그리고 Phase 5의 barge-in 완성과
D12 턴테이킹 튜닝까지. STT·Brain이 0초여도 현 CPU TTS로는 "명령 답변 ≤3초"가 **구조적으로
불가능**하다. 즉 **Phase 2 완료 기준과 Phase 5 절반이 하드웨어 한 건에 묶여 있다.**

**② Phase 3 완료 기준은 코드로 못 닫는다.** *"선제 발화 응답률이 무시율보다 높다"* 는
`interaction_log`가 쌓여야 판정되고, 그건 **실사용 기간이 전제**다(B2). 코드를 다 짜도
그날 닫히지 않는다 — 이건 지연이 아니라 기준의 성질이다. 그래서 Phase 3은
**"코드 완료"와 "기준 충족"을 따로 본다.**

**③ 사람이 내려야 할 결정 2건이 앞을 막았다 — 둘 다 해결(2026.08.28).**
- ~~**카드 "출처 작화"**~~ → **ⓒ 두 층으로 해결**(#46). 카드 `background`가 바깥소식을 아는
  **경로**를, `_PROACTIVE_FRAME`이 **보편 금지**를 맡는다. 프레임에 특정 카드의 세계관을 넣으면
  안 된다(한 번 넣었다가 되돌렸다) — `conductor.py`는 모든 카드가 공유하는 코드다. 상세 →
  [feed.md](./feed.md) §7.
- ~~**E5(언어를 페르소나 속성으로)**~~ → **카드 최상위 `language` 단일 출처로 해결**(#48).
  보류 사유였던 *"D3의 '언어는 가중치와 한 몸'과 충돌"* 은 **충돌이 아니었다** — 전 카드가
  `ref_lang == gen_lang`이라 원래 같은 값이고, 출처를 하나로 올리는 게 오히려 그 규칙을
  강제한다. D13 재료가 한국어 고정이라 선제 발화가 페르소나 언어를 이탈한 게 실해였다.

### 순서

| # | 작업 | 막는 것 | 규모 |
| :-- | :-- | :-- | :-- |
| ~~1~~ | ~~**D13 PR3**~~ — **완료(#47)**. config `feed:` + 데몬 배선(백그라운드 수집·후보 주입·kind 선택·`mark_used`) | — | 중 |
| ~~2~~ | ~~**A3 실기**~~ — **완료(2026.08.30).** fake mouth로 진짜 `_run`+tick 루프를 돌려 부팅→실 RSS 수집→선제 발화→기억 적재 확인. **마이크는 필요 없었다**(선제 발화엔 STT가 안 낀다). 이 실행이 PR3의 부팅 즉사 버그를 잡았다 — `_run` 스모크 부재가 드러남 | — | 소 |
| **3** | **B6 두뇌 벤더 결정** — 선제 발화 실측이 haiku를 배제하는 쪽을 가리킨다(자발 출처 언급 2/4). 대화 턴 품질 미검증 | — | 소 |
| 4 | **B5 / D11** — 스케줄 동기화(캘린더 vs 컴패니언 앱) | — | 중 |
| 5 | **B2** — `interaction_log` 축적 → hazard 튜닝 | **시간**(실사용) | 소, 상시 |
| — | *여기서 Phase 3 코드 완료. 기준 충족은 5가 쌓인 뒤* | | |
| 6 | **Phase 4** — 장기기억(D6 sqlite-vec) · 사실 충돌·망각 <br>~~D9 친밀도 산식 · 페르소나 전환 히스테리시스~~ **폐기(2026.08.31)** | — | 대 |
| 7 | **D8 확보** — GPU·상시기기 | 돈·하드웨어 | 외부 |
| 8 | **동결 해제** — C1 속도 · C2(D2 STT 선정) · C3 AEC | 7 | 중 |
| 9 | **Phase 5** — barge-in · D12 턴테이킹 · D10 정서 안전 · 고장 모드 · 상시기기 이전 | 7·8 | 대 |

**6(Phase 4)은 7(D8)을 안 기다린다** — 기억·인격은 전부 텍스트 계층이라 GPU와 무관하다.
D8이 늦어지면 **Phase 4를 먼저 하고 Phase 5를 뒤로 미는 순서**가 자연스럽다.

### 백로그 (v1 범위 밖 — 동결 유지)

D17(데몬 텍스트 대화 경로) · GUI 창 상태 영구화 · 페르소나 전환 로딩 표시 · TTS 엔진 핫스왑 ·
`TurnKind.PROACTIVE_CALLBACK`(PR3에서 재소환) · 안전 필터 본체.
...
| TTS (D3) | **GPT-SoVITS fine-tune** (아리스 168클립, Windows native CPU) | ✅ **확정 (2026.06.18)** |
| DB (D5) | SQLite | 사실상 확정 |
...
Gemini Live 등은 저지연·턴테이킹을 통째로 주지만, ① 목소리가 벤더 내장 음성에 묶여 나비의 연속성이 벤더 소유가 되고 ② 15분 세션 제한·세션 매니저 재설계가 필요하며 ③ 사용자 요구가 즉답(0.3초)이 아닌 ~1초 수준이라 직렬 스트리밍으로 충분. 대가로 턴테이킹을 자체 구현(01 문서 4.2절).
...
### TTS (D3) — ✅ 확정 (2026.06.18): GPT-SoVITS fine-tune
**GPT-SoVITS v2** 를 아리스 168클립(~14분)으로 fine-tune. Windows native 단일 프로세스 in-process 통합.
- **음색 = fine-tune 가중치가 안정적으로 담당.** 레퍼런스와 무관하게 "그냥 아리스" 음색 유지.
- **톤·억양 = 레퍼런스 오디오가 지배.** 무드별 레퍼런스 클립 풀로 상황에 맞는 톤 제어 가능.
- 실청취 결과: "집중 안 하면 동일인으로 착각" — 합격.
- Windows native CPU RTF 웜 **1.39** (WSL 측정치 1.4와 일치). 어댑터: `navi/mouth/gptsovits.py`.
- 클라우드 폴백(Cartesia 등)은 CPU RTF/TTFA 부족 판정 시 재검토.
...
재평가 결과 교체. sqlite-vec은 SQLite 확장이라 **장기기억 벡터가 같은 SQLite 파일 안에 들어감** → "나비의 기억 전체 = 파일 하나"가 되어 백업·이식·해자 관리가 극단적으로 단순해짐. 순수 C·무의존성이라 라즈베리파이에서도 동작. brute-force 검색이지만 개인 기억 규모(수천~수만 벡터)에선 ms 단위라 무관. 수백만 벡터 규모가 되면 그때 Chroma/pgvector로 이전.
