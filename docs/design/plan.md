...
**③ 사람이 내려야 할 결정 2건이 앞을 막는다.**
- **카드 "출처 작화"**([feed.md](./feed.md) §7) — 나비는 몸이 없어 집 밖에 못 나가는데 두뇌가
  "어제 뉴스에서 봤는데"라고 지어낸다. 재료가 진짜 뉴스가 되는 PR3에서 **매 선제 발화마다**
  터진다. 세계관 문제라 카드 소유자 판단.
- **E5(언어를 페르소나 속성으로 올릴지)** — `gen_lang`이 `voice.<vendor>` 안에 있어 voice 섹션
  없는 카드는 자기 언어를 선언할 수 없다. D3의 "언어는 가중치와 한 몸"과 충돌 가능. 실해가
  나기 전엔 급하지 않다.
...
| # | 작업 | 막는 것 | 규모 |
| :-- | :-- | :-- | :-- |
| 1 | **D13 PR3** — 데몬 배선 + `feed:` config | ③의 출처 작화 결정 · `create_brain` vendor 인자 | 중 |
| 2 | **A3 실기** — 음성 선제 발화 E2E | 1 | 소(사용자) |
