...
**E6-3. `config.local.yaml` 오버레이**(gitignore) — 머신 전용값이 갈 자리를 만든다.
`run_navi.ps1`의 `-VadThreshold 50`이 이 방·이 마이크 값인데(스크립트 주석이 명시) 커밋되는
파일에 박혀 있었다. config.yaml도 커밋 파일이라 거기 넣어도 남의 머신에 강요된다.
층위: **CLI > config.local.yaml > config.yaml > daemon 기본**. 첫 실사용값은 마이크 energy VAD.

**두 손잡이 구분:** `energy_vad_threshold`(마이크 프레임 → STT 게이트, `EnergyVad`)와
`wakeword.vad_threshold`(openWakeWord 내부 Silero, 호출어 오탐 억제,
[wakeword.py:209](../../navi/ear/wakeword.py:209))는 다른 층이다. 오버레이는 전자만 다룬다.

**"0 = VAD 없음"이 아니라 "미지정"이다.** 임계 0은 모든 프레임이 음성이라 발화 끝점이 안
잡혀 STT가 영영 안 돈다 — 시스템이 조용히 망가진다. 그래서 0은 daemon 기본(150,
`mic.py:46`·`listening.py:62`의 `vad or EnergyVad()`)으로 폴백한다.
(이전 문서의 "센티널화만으론 값이 사라진다=VAD 꺼짐"은 오독이었다.)
...
subprocess.Popen(
    ...
    ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass",
     "-File", str(script), "-Persona", persona],  # 목소리 없이는 ["-Mode","text"]
...
```
⚠ **원안의 `DETACHED_PROCESS`는 실물에서 실패했다(2026.07.21 구현 시 발견).** DETACHED는
콘솔을 아예 없애 run_navi.ps1의 `Write-Host`가 `ErrorActionPreference=Stop`과 만나 데몬 실행
**전에** 스크립트를 조용히 죽였다(powershell exit 0, 데몬 안 뜸). `CREATE_NO_WINDOW`는 숨은
콘솔을 줘 호스트가 살고, "GUI 죽어도 데몬 생존"은 부모 사망 시 자식 생존이 Windows 기본이라
DETACHED 없이도 성립한다. **비목표였던 관측성**: 유효성 확인을 위해 daemon이 유효 VAD 임계를
로그로 남기듯, 여기선 데몬의 `/status` 응답이 폴러의 유일한 성공 신호다.
...
**종료 (원안의 "정지 버튼 비목표"를 뒤집음, 2026.07.21):** 런처로 띄운 데몬은 숨은 콘솔이라
Ctrl+C할 터미널이 없고, index.html엔 종료 트리거가 없어 `-Mode stop`(터미널) 말곤 끌 방법이
없었다 — 런처의 "터미널 불필요" 가치가 종료에서 깨졌다. 그래서 **index.html 헤더에 종료 버튼**을
`POST /shutdown`에 배선한다(창 닫기 `close`와 별개 — 그건 GUI만 닫고 데몬은 산다). 되돌리려면
재기동+웜업이 필요하므로 **한 번 확인**. 종료 뒤 `return_to_launcher`가 대기 화면으로 되돌려
*끄기 → 대기화면 → 다른 엔진 → 실행* 루프를 완성한다(gui.md가 그리던 엔진 전환이 실제로 동작).
⚠ 죽어가는 데몬이 잠깐 더 `/status`에 답하므로, 복귀 폴러는 **데몬이 완전히 내려간 뒤** 무장한다
(死者로 되튀기 방지). "정지 버튼 비목표"는 이로써 폐기 — 단, **재시작 버튼은 여전히 안 만든다**
(종료+런처가 그 역할을 겸한다).
