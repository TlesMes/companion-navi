...
| 관심사 피드 (Feed) | **(Phase 3)** 뉴스·RSS·커뮤니티에서 선제 발화 주제 재료를 주기 수집 | — |
...
평소 닫혀 있고(STT 비활성) 웨이크워드로만 열려 endpoint/타임아웃에 닫힌다. **능동축**은
SLEEP/ACTIVE/DND/SNOOZE로, 나비가 *먼저* 말하는지를 가른다. 따라서 SLEEP/ACTIVE는
마이크 상태가 아니라 선제 발화 상태를 가리킨다.
...
> 창 단위 결정(D7 구현, 2026.06.25): **세션+무음 타임아웃(반려자식) 채택** — 한 번 깨우면 ACTIVE를
> 유지하며 여러 턴, active_timeout_ms 무음이면 SLEEP 복귀. 정체성("실시간 대화")상 세션형 선택.
> 타임아웃 값(기본 30초)은 실구동 튜닝(D12). 구현: navi/ear/listening.py(ListenSession).
...
| name | 프로필 이름. ~~같은 캐릭터의 단계별 프로필~~ — **D9 폐기로 카드당 1개 고정** |
| min_intimacy | ~~이 프로필이 적용되는 친밀도 하한~~ — **D9 폐기로 사문(死文). 항상 `0`** |
| background | 배경 서사 — 출신·역사·사용자와의 관계 설정 |
| traits | 성격·가치관·말투 규칙(어미, 호칭, 금지 표현) 서술 |
| example_dialogues | few-shot 대화쌍 10~20개 — 말투 형성의 핵심 재료 |
| dynamics | ~~**(Phase 4)** 성격 역학 계수 JSON — intimacy_gain_rate 등~~ — **D9 폐기로 함께 폐기.** 계수를 곱할 산식이 없어졌다 |

> **프로필은 카드당 1개다** (D9 폐기, 2026.08.31). 단계별 프로필은 작성자에게 "어느 단계의
> 대화문인지"를 상상하게 만들어 말투 일관성을 해친다 — 그게 폐기 이유 ①이다.
> Conductor가 카드를 시스템 프롬프트로 직렬화 — 캐싱 대상이라 카드가 커도 입력비 ≈ 0.

### intimacy — ✖ 폐기 (D9, 2026.08.31)
테이블·`get/update_intimacy`는 **코드에 남아 있으나 죽어 있다**: `update_intimacy`는 부르는 곳이
없고 `Conductor`는 항상 `0.0`을 읽어 첫 프로필만 고른다. 리포에 마이그레이션 경로가 없어
(`executescript` + `IF NOT EXISTS`) 테이블 제거는 기존 DB에 고아를 남기므로, **제거는 별도
판단**으로 둔다. 새 코드는 이 테이블을 읽지도 쓰지도 말 것.
...
| used_at | 선제 발화에 사용된 시각 (중복 발화 방지) |
...
| D7 | 웨이크워드 엔진 | ◐ **진행 중 (2026.06.25): 엔진=Vosk 채택(ASR 기반 스팟팅).** 청취축 상태머신·WakeWord 계약 완성(navi/ear/listening.py·wakeword.py) — 창=세션+무음 타임아웃. **엔진 경위:** Porcupine(진짜 음향 KWS)은 콘솔 가입이 회사 이메일을 요구해 개인 무료 불가 → 학습0·CPU·한국어 **Vosk**로 전환. **단, Vosk는 음향 KWS가 아니라 작은 ASR이다** — grammar 제한(`[unk]`)을 small-ko가 미지원해 무시 → **전체 인식 후 전사에서 호출어 포함 매칭**(검문①과 같은 방식, 모델만 경량). 따라서 **SLEEP은 'STT 꺼짐'이 아니라 '무거운 STT(whisper) 꺼짐 + 경량 ASR(Vosk) 켜짐'의 2단**(arch 5.1의 "STT off" 문구는 이 의미로 읽는다). Porcupine 어댑터는 벤더중립 증명으로 보존. 실측 인식률 향상(부분결과 감지·키워드 변이형·큰 모델)이 남은 튜닝(D12). 수면 명령은 검문① 텍스트 게이트 유지. 화자 인증은 v1 제외 — '아무나 깨어남' |
| D8 | 구동 하드웨어 | 보류. 추천: 1차 노트북/미니PC → 안정화 후 상시기기 (02 리포트) |
| D9 | 친밀도 산식 | ✖ **폐기 (2026.08.31) — v1 범위에서 제외.** 두 이유: ① **카드 설계가 예측 불가해진다** — 단계별 프로필을 쓰면 `example_dialogues`를 단계 수만큼 써야 하고, 작성자가 "어느 단계의 대화문인지"를 상상하며 일관성을 유지해야 한다. 말투 형성의 핵심 재료가 흔들린다. ② **오르내림 기준이 애매하다** — 무엇이 친밀도를 올리고 내리는지 종이로 정할 수 없고(`shared_vulnerability`를 몇 점으로?), 실사용 데이터로 튜닝하려 해도 페르소나가 이미 그 점수에 따라 변해 있어 **측정과 개입이 얽힌다**. 구 설계 요건(감쇠·히스테리시스·`persona.dynamics` 계수)은 이 표 아래 없음. 상세 → [progress.md](../progress.md) "D9 폐기" |
...
| D13 | 관심사 피드 소스 | ◐ **방향 확정 (2026.07.24, 설계 [feed.md](./feed.md)):** 소스=**RSS만**(뉴스 API·커뮤니티 크롤링 기각), 관심사 출처=**①명시 등록+②대화 빈도 자동추출**(②는 RSS 아닌 메모리 콜백), 안전 필터=**MVP 제외**(`_is_safe` 훅 씨앗만), 수집=**DaemonCore tick piggyback**(별도 스케줄러 없음). 구현은 B4(PR 3분할). |
...
| D16 | 모드 축 분리 (청취 ⊥ 능동) | (신규, 2026.06.25) **방향 정리.** 그동안 한 덩어리로 보던 모드를 두 직교 축으로 분리. **청취축**=마이크→STT→LLM 문: 평소 닫힘(STT 비활성), 웨이크워드(D7)로만 열려 endpoint/타임아웃에 닫힘. **능동축**=SLEEP/ACTIVE/DND/SNOOZE: 나비가 *먼저* 말하는지(`can_speak_now`). 따라서 SLEEP/ACTIVE는 마이크 상태가 아니라 **선제 발화 상태**를 가리킨다. 연결은 한 방향뿐 — 사용자 발화(웨이크워드)가 능동축을 ACTIVE로 승격(=강제기상). "자라"(검문①)는 능동축을 SLEEP으로 내리는 명령. 웨이크워드 창 단위는 **세션+무음 타임아웃 채택(D7 구현, 2026.06.25)** — 타임아웃 값만 D12 튜닝 |
| D17 | 데몬 텍스트 대화 경로 | (신규, 2026.07.20) **데몬엔 stdin 입력 경로가 없다** — 마이크·STT는 `if args.wakeword:` 안에서만 생성되고([navi/daemon.py:642](../../navi/daemon.py:642)) `console()`은 버스 구독자(출력 전용, [daemon.py:727](../../navi/daemon.py:727)). 따라서 웨이크워드 없는 기동은 대화가 아니라 **tick만 도는 점검용 골격**이다. 대화 경로를 여는 2단계: **1단계** `EventKind.RESPONSE`(턴당 1건, 완성 텍스트 — 토큰 단위는 버스 범람) 발행 + 기존 로그 다이얼로그에 표시, **본 레이아웃 무변경**. 음성 모드에서도 "실제 발화 문자열 확인" 진단 가치가 있어 텍스트 모드 전용으로 좁히지 않는다. **2단계** `POST /say` — STT만 건너뛰고 `check_gate`→`_run_turn` **동일 경로**(게이트를 우회하지 않는 것이 핵심) + 채팅 입력창. 2단계 착륙 시 런처(E6)의 "목소리 없이(점검용)" 라벨 개정 |
