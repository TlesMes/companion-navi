...
| D7 | 웨이크워드 엔진 | ◐ **진행 중 (2026.06.25): 엔진=Vosk 채택.** 청취축 상태머신·WakeWord 계약 완성(navi/ear/listening.py·wakeword.py) — SLEEP=호출어만/ACTIVE=대화 세션, 창=세션+무음 타임아웃. **엔진 경위:** Porcupine은 콘솔 가입이 회사 이메일을 요구해 개인 무료 불가 판명 → 학습0·CPU·한국어 모델 있는 **Vosk 키워드 스팟팅**으로 전환(제한 문법이라 whisper식 환각 없음, 검문①과 동일 철학). Porcupine 어댑터는 벤더중립 증명으로 보존, **Vosk 어댑터 구현이 다음**. 수면 명령은 검문① 텍스트 게이트 유지(KWS는 깨우기 전용). 화자 인증(주인만 깨움)은 v1 제외 — '아무나 깨어남' |
