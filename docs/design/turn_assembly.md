...
### 4.1 실 LLM 관측 (2026.07.25~26) — 확증 아님, 관측

파이프라인을 거치지 않고 `Conductor.build_request` → 두뇌 어댑터를 직접 호출해(일회용 프로브,
커밋 안 함) **같은 중립 시점 소재를 두 kind로 대조**했다. 소재 3종(날씨·스포츠·기술) × 2 kind,
성공 응답 **31건**을 §4 판정 기준(FAIL=소재를 사용자가 알려준 것으로 취급 / PASS=자기가 아는
것으로 먼저 꺼냄, **실행 전 고정**)으로 사람이 원문을 읽고 분류:

| 모델 | PROACTIVE | REACTIVE |
| :-- | :-- | :-- |
| gemini-3-flash-preview | 11/11 PASS | **0/8 PASS** (전부 FAIL) |
| claude-haiku-4-5 | 6/6 PASS | **0/6 PASS** (전부 FAIL) |

두 벤더·세 소재에서 분리가 예외 없이 나타났다. 전형적 FAIL: `"세상에, 34도면 뜨거웠겠다.
그래서 오늘 유난히 지쳐 보였던 거야?"` · `"오, 그랬구나. 너 그 경기 봤어?"` — 소재를 사용자
발화로 취급. 전형적 PASS: `"오늘 서울 낮 기온이 34도까지 올라간대. 나갈 때 조심해."`

**이 숫자의 한계(과신 금지):**
- **반복 표본이 독립이 아니다.** thinking을 끈 기본 샘플링이라 출력이 상당수 **글자 그대로
  중복**됐다(날씨 proactive 1·2 동일, 스포츠 reactive 1·2 동일 등). 유효 표본은 31보다 **작다** —
  "11/11"은 11번의 독립 시행이 아니다.
- **gemini 무료 티어가 하루 20콜에서 잘렸다**(429 `quotaValue: 20`,
  `GenerateRequestsPerDayPerProjectPerModel-FreeTier` — 분당이 아니라 **일일** 상한). 기술 소재의
  gemini 표본이 계획 5건 → 1건으로 줄었다.
- 소재는 **손으로 쓴 것**이지 실제 feed 요약이 아니다. 음성 E2E(A3)도 미검증.

따라서 이 결과가 지지하는 것은 **"결함이 실재하고 프레이밍이 그 방향으로 작동한다"**까지이며,
**"고쳤다"의 증명은 아니다.** 실 재료·음성까지 닫는 건 여전히 A3 트랙.

부수 확인: 무드 태그가 두 kind·두 벤더 모두 정상 출력 — 카드 코어의 출력 규칙이 kind와 무관하게
상속됨(§2 설계대로). 1차 프로브(2026.07.25, n=4)에서 입력 토큰 1142(REACTIVE) vs 1180(PROACTIVE)
— 차이는 프레이밍 ~38토큰뿐이고 **system은 동일**해 캐시 prefix 불변의 실측 근거.

**여기서 발견돼 feed로 넘긴 것 (2건):**
1. **소재 POV** — 1차 프로브에서 트리거를 `"네가 응원하는 야구팀"`(2인칭)으로 주자 나비가
   `"내가 응원하는…"`으로 재해석했다. 프레이밍 결함이 아니라 소재 문구의 시점 문제 →
   feed 요약은 **중립 시점**으로([feed.md](./feed.md) §3.3 반영 완료).
2. **출처 작화(作話)** — PROACTIVE 응답들이 정보를 어떻게 알았는지를 지어낸다:
   `"경기 결과 궁금해할 것 같아서 미리 알아봤어"`(gemini) · `"어제 뉴스에서 봤는데"`(haiku).
   나비는 카드상 **몸이 없어 집 밖에 못 나가고 집 안 소리만 듣는** 정령이라
   ([navi.yaml](../personas/navi.yaml) background) "봤다"·"알아봤다"는 **페르소나 위반**이다.
   프레이밍이 출처를 안 주니 모델이 빈칸을 메운 것. **feed 도입 시 나비가 바깥소식을 아는
   경위에 카드 차원의 서사가 필요**하다(feed.md §7 열린 질문에 등록).
