...
- **프레이밍 문구 최적화** — 값은 배선용. 실 재료 A/B 후 튜닝. **알려진 손잡이 2개**(§4.1에서
  드러남): ①**출처** — 현재 문구는 나비가 소식을 어떻게 아는지 안 알려줘 모델이 지어낸다
  ("뉴스에서 봤는데"). ②**여는 자세** — `"짧게, 되묻지 말고"`가 되묻기는 막지만 문장이 여는
  모양인지 받는 모양인지는 통제하지 않아 공감 우선("더웠겠다")이 섞인다. 둘 다 A/B 대상이지
  지금 고칠 것 아님(표본 부족 → 과적합 위험).
