...
### 4.1 실 LLM 확증 (2026.07.25, gemini-3-flash-preview)

구현 직후 파이프라인을 거치지 않고 `Conductor.build_request` → `GeminiBrain.generate_stream`을
직접 호출해(일회용 프로브, 커밋 안 함) **같은 서술형 소재를 두 kind로 대조**했다. §1의 가설이
그대로 재현됐다 — 특히 두 번째 사례가 결정적:

| 소재 | REACTIVE (결함 재현) | PROACTIVE (교정) |
| :-- | :-- | :-- |
| 34도 폭염 | "세상에, 34도나 된다고? 어쩐지 아까부터…" — 사용자가 알려준 걸로 **반응** | "오늘 서울은 낮 기온이 34도까지 올라간대. 나갈 때 조심해." — 자기가 알게 된 소식으로 **먼저 꺼냄** |
| 야구팀 5대2 승 | "오, 정말? … **그런데 내가 응원하는 팀이 어디야?**" — 소재를 사용자 발화로 오해해 **되물음** | "어제 내가 응원하는 야구팀이 다섯 점이나 내고 이겼어. 기분 좋아서 들떠 있었네." — **먼저 꺼냄** |

부수 확인: ①무드 태그가 두 kind 모두 정상 출력(`[mood:neutral]`·`[mood:bright]`) — 카드 코어의
출력 규칙이 kind와 무관하게 상속됨(§2 설계대로). ②토큰은 입력 1142(REACTIVE) vs 1180(PROACTIVE)
— 차이는 프레이밍 ~38토큰뿐이고 **system은 동일**해 Gemini implicit caching 대상(캐시 prefix
불변의 실측 근거).

**여기서 발견돼 feed로 넘긴 것:** 트리거를 `"네가 응원하는 야구팀"`(2인칭)으로 주자 나비가
`"내가 응원하는…"`으로 재해석했다. 프레이밍 결함이 아니라 **소재 문구의 시점(POV) 문제** —
feed 요약 LLM이 후보 summary를 **중립 시점**으로 뽑아야 한다([feed.md](./feed.md) §3.3 요약
프롬프트에 반영할 제약).
