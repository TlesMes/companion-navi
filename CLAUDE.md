...
**두뇌를 GUI에서 갈아끼운다 — 재기동 없이, 선택은 남는다(2026.09.01).** 헤더 톱니 → 설정
팝업에서 벤더를 고르고 키를 넣으면 검증 후 교체된다([navi/control/runtime.py](./navi/control/runtime.py)
`swap_brain` + `/brain`). **설정 층위가 4층이 됐다** — `config.yaml < config.local.yaml <
DB(`setting` 테이블) < CLI`. 이로써 **gui.md:121의 구멍(런타임 변경이 재기동 시 config로 복귀)이
닫혔다**(A2가 취침창으로 실증했던 부채). **키는 `.env`에 남는다** — `navi.db`는 백업·이관 때
통째로 복사하는 기억 파일이라 비밀의 자리가 아니다. 교체는 **검증 성공 뒤에야 커밋**한다
(실패 시 `.env`·`setting`·기존 두뇌 전부 무변화). 요약기는 별도 인스턴스라 교체 대상이 아니다.
446 tests green. 상세 → [progress.md](./docs/progress.md) 최상단.
...
**A3 통과 — 나비가 실제로 스스로 말했다(2026.08.30).** fake mouth로 진짜 `_run` + tick 루프를
돌려 부팅 → 실 RSS 수집 → 요약 → 후보 적재 → 게이트 통과 → 선제 발화 → 기억 적재를 한 번에
확인했다. **마이크는 필요 없었다** — 선제 발화엔 STT가 안 낀다(마이크는 *반응* 측정 = B2 영역).
TTS는 텍스트와 완전 일치(supertonic RTF 0.33~0.41 / gptsovits 1.78~2.01).

**★ 그 실행이 PR3의 부팅 즉사 버그를 잡았다.** `_build_feed`가 `user_id` 정의보다 두 줄 앞에
있어 데몬이 어떤 모드로 뜨든 죽었는데 **411 tests는 전부 green이었다** — 통합 테스트가
`DaemonCore`에 협력자를 직접 주입해 부팅 경로(`_run`)를 건너뛴다. **`_run` 스모크도 린터도
없다**(백로그 등록). 부품 검증과 조립 검증은 다르다.

**남은 것은 B6(두뇌 벤더 결정) 하나** — 선제 발화 실측에서 haiku만 자발 출처 언급 2/4·질문끝
1/4을 어긴다(sonnet·gemini 0/4). 대화 턴 품질은 미검증. **비교 왕복은 싸졌다** — 이제 같은
세션에서 GUI로 두뇌만 갈아가며 이어 말해 볼 수 있다(위 문단). 상세 →
[progress.md](./docs/progress.md) 최상단.

**PR3 선행은 #46에서 완료(2026.08.25).** 출처 작화(*"어제 뉴스에서 봤는데"*)는 **ⓒ 두 층**으로
닫았다 — 카드 `background`가 **경로**를(*"바깥 이야기가 흘러들어오되 어디서 왔는진 모른다"*),
`_PROACTIVE_FRAME`이 **보편 금지**를 맡는다(지어내지도, 굳이 밝히지도 말 것).
**프레임에 특정 카드의 세계관을 넣으면 안 된다** — `conductor.py`는 모든 카드가 공유하는 코드다
(한 번 넣었다가 되돌렸고 회귀 테스트로 잠갔다). 대화 콜백용 `TurnKind.PROACTIVE_CALLBACK`도 추가.
**남은 PR3 선행은 `create_brain(config, *, vendor=None)` 하나.**
...
**D7(웨이크워드): 확정.** 엔진=**openWakeWord**(Vosk 폐기 — ASR 스팟팅은 진짜 KWS 아님, 피벗 경위 progress.md Stage 6·7). 한국어 "나비야" 모델은 livekit-wakeword(VoxCPM2 합성)로 학습 → **`assets/wakeword/navi_ko.onnx`(커밋 자산 — E6-1에서 secrets/에서 이관, 자체 학습물이고 없으면 `--wakeword` 기동이 죽는다)**(conv_attention, 다리 없이 기존 어댑터 그대로 로드) → **원어민 마이크 실측 통과**([navi/ear/wakeword.py](./navi/ear/wakeword.py)). 임계값 튜닝은 후순위.
