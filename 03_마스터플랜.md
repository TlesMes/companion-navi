...
> 상세 설계는 [01_설계_데몬_아키텍처.md](./01_설계_데몬_아키텍처.md), 벤더 조사·원가는 [02_기술조사_원가_리포트.md](./02_기술조사_원가_리포트.md) 참조.
...
| STT (D2) | Deepgram Nova-3 스트리밍 | 유력 (재평가 후 유지) |
| TTS (D3) | Cartesia Sonic 3.5 vs ElevenLabs Flash | **미정 — 최중요 결정** |
| DB (D5) | SQLite | 사실상 확정 |
...
### STT (D2) — Deepgram Nova-3 스트리밍 (재평가 후 유지)
스트리밍 모드 sub-300ms, **한국어가 Nova-3 확장에서 최대 수혜 언어(WER 27% 개선)**, $0.0077/분. 재평가에서 나온 Deepgram Flux(모델 기반 턴 감지 내장 — D12 부담을 크게 줄여줌)는 **한국어 미지원(2026.6 현재 10개 언어)**이라 탈락. **Flux가 한국어를 지원하는 시점에 재검토할 것** — 턴테이킹의 절반을 STT가 흡수해주는 구조라 가치가 큼.
